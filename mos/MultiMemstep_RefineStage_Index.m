      ...
      Mem_Index = Nstep;
      Mem_I     = Nstep;
      GPU_Mem            =  zeros(1,Mem_Index+1);
      S_Mem              =  zeros(1,Mem_Index+1);
    ...
    Area_sum = Area_sum  -  upper_limits(i);
     ...
     for Mem_I  = 1 :(Nstep+1)
           ...
           if Mem_I >=2
               ...
               if Mem_I >3
                 Refine_Ratio = 1;
                 y_feed(Mem_I)    = Refine_Ratio*FracP_py(Mem_I-1);
                 F_feed(Mem_I)    = f_perm(Mem_I-1);  
                 P_Memfeed(Mem_I) = P_Memfeed(1);
                 Area(Mem_I)      = Area_refine;
               end
           end
...
%     [T_out, P_out, F_out, Frac1, Frac2]=Conpressure(t_feed1, P_resd_out1, f_resd_1 , FracR_py1, FracR_pa1); %% Increase the pressure  
     ...
     f_perm_downstream_Purity_py   =  f_perm_downstream_py_sum/f_perm_downstream_sum;
     f_perm_downstream_Recovery_py =  f_perm_downstream_py_sum/(y_feed(1)*F_feed(1)); 
    
    
     xlswrite('C:\Users\chang\OneDrive\Documents\Graduate level documents\Master thesis\gnh use\Two step Mem 1.xlsx',Area                ,'sheet4',['A',num2str(Index),':C',num2str(Index)]); 
     xlswrite('C:\Users\chang\OneDrive\Documents\Graduate level documents\Master thesis\gnh use\Two step Mem 1.xlsx',GPU_Mem             ,'sheet4',['D',num2str(Index),':F',num2str(Index)]); 
     xlswrite('C:\Users\chang\OneDrive\Documents\Graduate level documents\Master thesis\gnh use\Two step Mem 1.xlsx',S_Mem               ,'sheet4',['G',num2str(Index),':I',num2str(Index)]); 
     xlswrite('C:\Users\chang\OneDrive\Documents\Graduate level documents\Master thesis\gnh use\Two step Mem 1.xlsx',P_resd_out          ,'sheet4',['J',num2str(Index),':L',num2str(Index)]); 
