...
clear
      ...
      Work_space1        = zeros( span ,27);
      Work_space2        = zeros( span ,42);
      Work_space3        = zeros( span ,5);
      % Price              = cell(1,4);
      write_space1       = 1;
      write_space2       = 1;
      % Price              = zeros(1, 2);  % Initialize Price array
      price_select       = zeros(1, 2);
      Index_Mem          = 3;
      interval           =  span ;
      ad_i = 5;
        % for read_i = 4
            read_i = 25+ad_i;
      %% MutipleStep readdata path 
      ...
      Read_Data = readmatrix( path   ,'Sheet',Sheet_value2,'Range',  ['A' ,num2str(Index_Mem )  ,':',alpha_quire( Nstep*11+3) ,num2str(Index_Mem  + interval-1)]);
  ...
  Price = [194,194];
    ...
    Read_Data_subset = Read_Data(Read_Index, 5:6);
    Price = [194,194] .* (Read_Data_subset <= 31) + Price2 * (Read_Data_subset > 31);


      P_resd_connect     =   Read_Data (Read_Index, Nstep*10-6);
      P_perm_connect     =   10;
      f_perm_py_connect  =   Read_Data(Read_Index,Nstep*10+1) * Read_Data(Read_Index,Nstep*11+3)/100;
      f_perm_pa_connect  =   Read_Data(Read_Index,Nstep*10+1) * (100-Read_Data(Read_Index,Nstep*11+3))/100;
      f_resd_py_connect  =   Read_Data(Read_Index,Nstep*10-2) * Read_Data(Read_Index,Nstep*10-1);
      f_resd_pa_connect  =   Read_Data(Read_Index,Nstep*10-2) * Read_Data(Read_Index,Nstep*10);
    ...
    %% of TCC
    Cost_Mem  = (A1*Price(1) +  A2*Price(2))/10000;
    ...
    interval      =  span ;
    Index_couple  = 4;
    path2 = 'C:\Users\chang\OneDrive\Documents\Graduate level documents\Master thesis\gnh use\Energy Edit fund11.xlsx';
    sheet_value2 = 29+ad_i;
    sheet = ['Sheet',num2str(sheet_value2)];
    range2 = ['A' ,num2str(Index_couple )  ,':  AP' ,num2str(Index_couple  + interval)];
    range3 = ['AR' ,num2str(Index_couple )  ,':  AV' ,num2str(Index_couple  + interval)];
