...
%% 导入MATLAB数据
    ...
    spreadb = hyspread(hysys, 'SPRDSHT-1');          % Create a connection to a spreadsheet.
    m = hycell(spreadb, {'c1','c2','c3','c4','d3'}); % Connect to the cells  in the spreadsheet.
    mm = hyvalue(m);
    f_feed = mm(1);
    P_feed = mm(2);%kpa
    y_feed1 = mm(3);
    ...
    y_feed3 = y_feed2(2);
    kk = kk + 1;
    [f_c3h6_resd, f_c3h8_resd, f_c3h6_perm, f_c3h8_perm, P_resdN] = Solve_Mem_Op(J1, S12, A, f_feed, P_feed, y_feed1, y_feed3, N);
