%% 参数设定
    ...
    %% 初始化变量
    y_perm = zeros(3);             % 渗透端各组分摩尔分率
    F_perm = zeros(3);             % 渗透端每个微元流量
    total_perm_flow = 0;
    ...
    deltaP = (P_feed - P_perm) ;  % 压力差均匀分布
        ...
        for j = 1:3
            flux = P_perm_coeff(j) * A * (P_feed - P_perm) * y_feed(j);  % 单位：kmol/m²·h·Pa
            F_perm(j) = flux;  % 计算每个微元的渗透流量
            total_perm_flow = total_perm_flow + F_perm(j);  % 累加总渗透流量
        end
        % 计算渗透端各组分组成
        y_perm(:) = F_perm(:) / total_perm_flow;
