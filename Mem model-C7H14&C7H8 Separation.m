...
%%------------------------------------------------------------------------------%%
  ...
  % 预分配数组和变量声明===================================================================================================
    ...
    R =  zeros(NStage,nComp); %% Recovery  回收率
    P =  zeros(NStage,nComp); %% Purity 纯度
   %%=====================================================================================================================
   
   

    % %%参数写入EXCEL 路径说明==============================================================================================%%
    filename_Data = 'C:\Users\AIBOOM\OneDrive\Documents\Graduate level documents\Master thesis\SEP C7H14-C7H8\OutputData.xlsx';
    Write_Sheet   = 1;
    ...
    P_perm = 0.01;                           % 渗透侧压力，Pa (1 bar)
                                             % 膜面积
    components = 2;                          % 组分数
    A1 = 0; A2 = 0;
    A1_list = 100:100:1000;                  % 一级膜面积扫描
    ...
    for A = A1_list
        ...
        NStage_Fill = 1;
           ...
           % 第一阶段
            [y_perm(NStage_Fill,:),y_resd(NStage_Fill,:),F_perm(NStage_Fill,:),F_resd(NStage_Fill,:),A_used] = spice_unit(y_feed,F_feed,P_feed,P_perm,A,components,PERMEANCE_GPU1);
            F_resd_sum(NStage_Fill) =  sum(F_resd(NStage_Fill,:)); 
           
            % 一级膜回收率与纯度
            R(tick,NStage_Fill)     =  F_resd_sum(NStage_Fill)/(F_feed * y_feed(1));  %% MCH Recovery  回收率
            ...
            Aspen.Application.Tree.FindNode('\Data\Streams\F1\Input\TOTFLOW').Value             = F_resd_sum(NStage_Fill);
            Run2(Aspen.Engine);
            %%% Aspen 计算结果获取 
            VCCU_DUTY(1,NStage_Fill) = Aspen.Application.Tree.FindNode('\Data\Blocks\VCCU1\Output\BRAKE_POWER').Value;
            COOL_DUTY(1,NStage_Fill) = Aspen.Application.Tree.FindNode('\Data\Blocks\COOL1\Output\BRAKE_POWER').Value;
            %%  参数计算和汇总

            COOL_DUTY_Total          = sum(COOL_DUTY);
            VCCU_DUTY_Total          = sum(VCCU_DUTY);
            database(tick,:) = [A1, A2, R(tick,1 ), P(tick,1 ), R(tick,2 ), P(tick,2 ) , VCCU_DUTY_Total, COOL_DUTY_Total];
           
            
      
      %% 二级和多级膜计算

      if y_resd(1,1) < target_purity
         
          NStage_Fill = 2;
          for A2 = A2_list
            %% 计算第二级膜上渗透性
              PERMEANCE_GPU2 = Flux_model(filename, membrane_id, y_resd(1,1));
            ...
            % 二级膜回收率与纯度
            F_resd_sum(NStage_Fill) =  sum(F_resd(NStage_Fill,:)); 
            R(tick,NStage_Fill)         =  F_resd_sum(NStage_Fill)/(F_resd_sum(NStage_Fill-1) * y_resd(NStage_Fill-1,1));      %% MCH Recovery  回收率
            P(tick,NStage_Fill)         =   y_resd(NStage_Fill,1);                                                           %% MCH Purity    纯度
        ...
        COOL_DUTY(1,NStage_Fill)   = Aspen.Application.Tree.FindNode('\Data\Blocks\COOL2\Output\BRAKE_POWER').Value;
        %%  参数计算和汇总
        COOL_DUTY_Total              = sum(COOL_DUTY);
        VCCU_DUTY_Total              = sum(VCCU_DUTY);
        database(tick,:) = [A1, A2, R(tick,1 ), P(tick,1 ), R(tick,2 ), P(tick,2 ) , VCCU_DUTY_Total, COOL_DUTY_Total];
